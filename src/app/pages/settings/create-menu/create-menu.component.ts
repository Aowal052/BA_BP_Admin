...
import { Menu, MenuResponse } from 'src/app/@core/model/MenuResponse';
...
export class CreateMenuComponent implements OnInit{
  ...
  perentMenuDropdown: { id?: number, name?: string }[] = [];
  moduleIdDropdown: { id?: number, name?: string }[] = [];
  ...
  formConfig: FormConfig = {
    ...
    items: [
      ...
      },
      {
        ...
        prop: 'parentMenuId',
        type: 'select',
        options:  this.perentMenuDropdown,
        required: true,
        rule: {
          validators: [{ required: true }],
        },
  ...
  async getParentMenu(){
    (await this.menuService.getParentMenu(ApiEndPoints.GetAllMenuParent)).subscribe((response:MenuResponse) => {
      debugger
      this.res = response;
      if(this.res.statusCode == HttpStatusCode.Ok){
        this.parentMenu = this.res.data;
        this.perentMenuDropdown = this.parentMenu.map(item => ({ id: item.id, name: item.title }));
      }
    });
  }
  ...
  updateFormConfigOptions() {
    debugger
    //9this.formConfig.items.find((item: { prop: string; }) => item.prop === 'KeyAccountManager').options = this.categoryDropdown;
  }
