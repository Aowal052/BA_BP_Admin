...
export class LoginComponent implements OnInit {
  ...
  onClick(tabId: string | number) {
    const formData = new FormData();
      formData.append('UserId', this.formData.userAccount||'');
      formData.append('UserPassword', this.formData.userAccountPassword||'');
    switch (tabId) {
      case 'tab1':
        this.authService.login(formData,ApiEndPoints.Login).subscribe(
          (res) => {
            debugger
